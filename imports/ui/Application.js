...
export class Application extends React.Component {
  ...
  onPhoneChange(index, e) {
    ...
    if (validator.isMobilePhone(e.target.value, "en-US")) {
      ...
      if (
        !field.classList.contains("validation-error") &&
  ...
  onDobChange(e) {
    ...
    if (validator.isISO8601(dob) && dob.length === 10) {
      if (
        Number(dob.substring(0, 4)) > 999 &&
        Number(dob.substring(5, 7)) > 0 &&
        Number(dob.substring(5, 7)) < 13 &&
        Number(dob.substring(8)) > 0 &&
        Number(dob.substring(8)) < 32
      ) {
        if (field.classList.contains("validation-error")) {
          field.classList.remove("validation-error");
        }
  ...
  onTournamentSelection(e) {
    document
      .querySelector("#tournaments-field")
    ...
    for (let i = 0; i < options.length; i++) {
      if (options[i].selected) {
        selectedTournaments.push({
          ...
          tournamentName: this.state.tournaments.find(
            (tournament) => tournament._id === options[i].value
          ).name,
          divisions: this.state.tournaments.find(
            (tournament) => tournament._id === options[i].value
          ).divisions,
          weightClasses: this.state.tournaments.find(
            (tournament) => tournament._id === options[i].value
          ).divisions[0].weightClasses,
          selectedDivision: this.state.tournaments.find(
            (tournament) => tournament._id === options[i].value
          ).divisions[0].name,
          selectedWeightClass: this.state.tournaments.find(
            (tournament) => tournament._id === options[i].value
          ).divisions[0].weightClasses[0],
        });
      }
    }

    for (let j = 0; j < selectedTournaments.length; j++) {
      const prevSelection = prevSelections.find(
        (prev) => prev.tournamentId === selectedTournaments[j].tournamentId
      );

      if (prevSelection) {
        selectedTournaments[j].weightClasses = prevSelection.weightClasses;
        selectedTournaments[j].selectedDivision =
          prevSelection.selectedDivision;
        selectedTournaments[j].selectedWeightClass =
          prevSelection.selectedWeightClass;
      }
    }
  ...
  onDivisionSelection(selectedTournament, e) {
    ...
    for (let i = 0; i < selectedTournaments.length; i++) {
      if (
        selectedTournaments[i].tournamentId === selectedTournament.tournamentId
      ) {
        selectedTournaments[i].selectedDivision = e.target.value;
        selectedTournaments[
          i
        ].weightClasses = selectedTournament.divisions.find(
          (division) => division.name === e.target.value
        ).weightClasses;
        selectedTournaments[
          i
        ].selectedWeightClass = selectedTournament.divisions.find(
          (division) => division.name === e.target.value
        ).weightClasses[0];
      }
    }
  ...
  isValidApplication(wrestler) {
    ...
    if (!validator.isISO8601(wrestler.dob) || wrestler.dob.length !== 10)
      return false;
    if (
      validator.isEmpty(wrestler.grade) ||
      wrestler.grade < 0 ||
      wrestler.grade > 12
    )
      return false;
    if (
      validator.isEmpty(wrestler.weight) ||
      wrestler.weight < 0 ||
      wrestler.weight > 285
    )
      return false;
    ...
    if (!wrestler.emails.every((email) => validator.isEmail(email.text)))
      return false;
    if (
      !wrestler.phones.every((phone) =>
        validator.isMobilePhone(phone.number, "en-US")
      )
    )
      return false;
  ...
  showValidationErrors(wrestler) {
    ...
    if (!wrestler.parents.every((parent) => !validator.isEmpty(parent.name))) {
      wrestler.parents.forEach((parent, index) =>
        document
          .querySelector("#parent-field-" + index)
          .classList.add("validation-error")
      );
    }
    if (!wrestler.emails.every((email) => validator.isEmail(email.text))) {
      wrestler.emails.forEach((email, index) =>
        document
          .querySelector("#email-field-" + index)
          .classList.add("validation-error")
      );
    }
    if (
      ...
      wrestler.parents.forEach((phone, index) =>
        document
          .querySelector("#phone-field-" + index)
          .classList.add("validation-error")
      );
    }
  ...
  render() {
    return (
      <div
        id="wrestler-application"
        className="container container__application"
      >
        ...
        <div className="container__content container__application-form">
          ...
          <label>
            {this.state.isDateSupported ? (
              ...
              <p>
                Date of Birth <span>(formatted as: YYYY-MM-DD)</span>
              </p>
            )}
            <input
              id="dob-field"
              ...
              if (index === 0) {
                return (
                  <div key={index} className="dynamic-field">
                    <input
                      ...
                      name="parent"
                      value={this.state.parents[index].text}
                      placeholder="Parent"
                      onChange={this.onParentChange.bind(this, index)}
                    />
                    <img
                ...
                return (
                  <div key={index} className="dynamic-field">
                    ...
                    />
                    <img
                      src="/images/remove-button.svg"
                      onClick={this.onDeleteParent.bind(this, index)}
                    />
                  </div>
                );
              }
                ...
                return (
                  <div key={index} className="dynamic-field">
                    <input
                      ...
                      name={"email-" + index}
                      type="email"
                      value={this.state.emails[index].text}
                      placeholder="Email"
                      onChange={this.onEmailChange.bind(this, index)}
                    />
                    <img
                      src="/images/add-button.svg"
                ...
                return (
                  <div key={index} className="dynamic-field">
                    <input
                      ...
                      placeholder="Phone"
                      onChange={this.onPhoneChange.bind(this, index)}
                    />
                    <img
                      src="/images/add-button.svg"
          ...
          <label className="bordered">
            ...
            <select
              id="tournaments-field"
              name="tournament"
              className="multi"
              size={this.state.tournaments.length}
              value={this.state.selectedTournaments.map(
                (selectedTournament) => selectedTournament.tournamentId
              )}
              onChange={this.onTournamentSelection}
              multiple
            >
            ...
            return (
              <fieldset key={selectedTournament.tournamentId}>
                ...
                <label>
                  <p>Division</p>
                  <select
                    id="selected-tournament-division"
                    name="selected-tournament-division"
                    value={selectedTournament.selectedDivision}
                    onChange={this.onDivisionSelection.bind(
                      this,
                      selectedTournament
                    )}
                  >
                ...
                <label>
                  <p>Weight Class</p>
                  <select
                    id="selected-tournament-weight-class"
                    name="selected-tournament-weight-class"
                    value={selectedTournament.selectedWeightClass}
                    onChange={this.onWeightClassSelection.bind(
                      this,
                      selectedTournament
                    )}
                  >
                    {selectedTournament.weightClasses.map(
                      (weightClass, index) => {
                        return (
                          <option key={index} value={weightClass}>
                            {weightClass}
                          </option>
                        );
                      }
          ...
          <button className="submit-button" onClick={this.onSubmitApplication}>
            Submit Application
          </button>
          ...
          <Modal
            ...
            className="boxed-view__box unbounded-height"
                ...
                {this.state.selectedTournaments.map((selectedTournament) => {
                  return (
                    <li key={selectedTournament.tournamentId}>
                      ...
                      {selectedTournament.selectedWeightClass} lbs.)
                    </li>
                  );
                })}
              </ul>
              <button
                className="button boxed-view__button-confirmation"
...
export default createContainer(() => {
  ...
  const tournaments = Tournaments.find({ published: true })
    .fetch()
    .map((tournament) => {
      return {
        ...tournament,
