...
export const Wrestler = (props) => {
  const convertGrade = (grade) => {
    switch (grade.toUpperCase()) {
      ...
      case '10':
      case '11':
      case '12': return grade + 'th grade';
      default: return null;
    }
  };

  const className = props.wrestler.selected ? 'item item--selected' : 'item';
  const weightSubtitle = props.wrestler.weight ? props.wrestler.weight + ' lbs.' : 'Weight';
  let gradeSubtitle = convertGrade(props.wrestler.grade);
  gradeSubtitle = gradeSubtitle ? gradeSubtitle : 'Grade';
